;;;; build-x64-cl-repl.lisp — the first BARE-METAL image that runs the REAL
;;;; CL (task #204 step 1).
;;;;
;;;; A multiboot x86-64 kernel for QEMU that drops into Modus's own self-hosted
;;;; Common Lisp REPL over the COM1 serial port: the CL reader, `eval' =
;;;; mvm-eval (compile -> MVM bytecode -> mvm-interpret), and the CL printer.
;;;; There is NO second Lisp here — `mvm/repl-source.lisp' (the 708-line toy
;;;; reader/printer/tree-walker baked by build-x64-repl and 23 other scripts)
;;;; is not loaded and not referenced.
;;;;
;;;; Lineage: this is `build-generic-cli.lisp' (the clean hosted CLI image)
;;;; retargeted to bare metal.  Same CL stack, same in-image compiler, same
;;;; scanners.  The deltas are:
;;;;   - boot/boot-x64.lisp + (build-image :target :x86-64)  [multiboot @1MB]
;;;;   - bare-metal x64 translator mode (port I/O, no Linux syscalls), NX on
;;;;     data pages, stack relocated above the image
;;;;   - the hosted payload is dropped: hosted-sockets / hosted-storage /
;;;;     hosted-http / lib/cli-toplevel.lisp (argv + fd-0 stdin + ~/.modusrc)
;;;;     and tar / install-tarball
;;;;   - the JIT is forced OFF (it mmap/mprotects a page; bare metal has no
;;;;     mmap — a later rung)
;;;;   - a serial REPL driver replaces (cli-toplevel): the character source is
;;;;     `read-char-serial' (TRAP #x0301 = poll COM1 LSR, IN from 0x3F8), the
;;;;     rest is the real CL runtime.
;;;;
;;;; STATUS (measured in QEMU, not inferred).  Working: boot + prompt; (+ 1 2)
...
(format t "~%=== Building bare-metal x64 CL REPL image ===~%")
...
(defvar *driver-source*
 (concatenate 'string "
;; BARE METAL: no process model.  HLT in a loop wakes on every PIT IRQ and
...
(defun kernel-main ()
  ...
  ;; global-alist head.  (boot-x64.lisp already zeroes 0x10000180 /
  ;; 0x10000400 / 0x10000C70.)
...
;; Bare-metal x64 boot descriptor (multiboot, kernel at 0x100000, no Linux).
(mvm-load "boot/boot-x64.lisp")

(in-package :modus.mvm)

;; STACK RELOCATION: the default stack top 0x800000 sits INSIDE the loaded
;; image (which starts at 0x100000 and is tens of MB), so the stack would
;; silently shred native code below it.  Move it to the top of QEMU -m 512 RAM,
;; above the heap + MCGC metadata: [heap-end 0x1DFFF000 | meta 0x1E000000.. |
;; ... stack grows down ... | 0x20000000).  boot-x64.lisp threads this through
;; both mov-esp/rsp sites AND the GC stack_base slot (0x10000058).  REQUIRES
;; -m 512.  (Same reasoning as build-x64.lisp.)
(setf modus.mvm::*x64-stack-top-override* #x20000000)

;; NX on data pages: the REPL will happily be handed (funcall 42) by a user.
;; With an RWX identity map that jump EXECUTES heap garbage and triple-faults
;; the machine; with NX on phys >= 0x10000000 the instruction fetch #PFs into
;; boot-x64.lisp's IDT-14 recovery, which longjmps to the innermost armed
;; handler-case — i.e. the REPL prints an error and keeps going.
(setf modus.mvm::*x64-nx-data-enable* t)

;; Install the x64 translator in BARE-METAL mode: *x64-linux-mode* NIL makes
;; the TRAP codes emit port I/O (OUT/IN on COM1) instead of Linux syscalls —
;; which is exactly what read-char-serial / write-char-serial need.
...
;; NOTE: *mcgc-kind-bitmap-enabled* stays NIL — the kind-bitmap base delta is a
;; boot-linux-x64 layout constant; the bare-metal metadata region at 0x1E000000
;; does not carry it.  No *linux-x64-r14-offset* either: bare-metal R12/R14 are
;; set by boot-x64.lisp's kernel64 entry (semispaces [0x10001000, 0x17000000) /
;; [0x17000000, 0x1DFFF000), R14 = 0x17000000).

;; Native code offset for fn-entry alignment: MEASURED by emitting the three
;; boot stages (multiboot header + boot32 + kernel64 entry) into a scratch
;; buffer, + 5 for the JMP rel32 cross.lisp inserts before native code.
;; Measuring keeps this correct across boot-x64.lisp changes (a stale constant
;; here breaks fn alignment -> deterministic NIL-deref crashes; see
;; reference_boot_preamble_size_load_bearing).
...
(format t "~%Compiling bare-metal CL REPL image (~D chars)...~%"
...
(let ((image (build-image :target :x86-64
  ...
  (let ((path (or #+sbcl (sb-ext:posix-getenv "MODUS_CL_REPL_OUT")
    ...
    (format t "Run: qemu-system-x86_64 -kernel ~A -m 512 -display none -serial stdio -no-reboot~%"
            path)))