...
ev("(setq *jit-hot-only* nil)"); ev("*jit-linkage-cells*"); ev("(jit-eager)",2400.0)
...
ev("(list *rh-plane* *hvs-kernel-slot* (hvs-displist) (hvs-active-channel))",30.0)
ev("(progn (hvs-window-nc nil) (let ((r nil)) (dotimes (i 30) (push (mem-ref (+ (hvs-base) 8192 (* (+ *rh-plane* i) 4)) :u32) r)) (reverse r)))",60.0)
ev("(progn (hvs-window-nc nil) (let ((r nil)) (dotimes (i 11) (push (mem-ref (+ (hvs-base) 8192 (* (+ *hvs-kernel-slot* i) 4)) :u32) r)) (reverse r)))",60.0)
ev("(let ((ptrs (list 340025376 340114448 340139024))) (list (logior 3221225472 (first ptrs)) (logior 3221225472 (second ptrs))))")
print("=== DLIST DONE ===",flush=True)
