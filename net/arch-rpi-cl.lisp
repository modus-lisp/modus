...
;; ============================================================
;;
;;   0x20000000  USB control-transfer DMA + E1000-shaped RX descriptor base
;;               (+0x000 setup buf, +0x040 data buf, +0x800 USB state)
;;   0x20001000  RX frame buffer (2 KB, the CDC bulk-IN landing zone)
;;   0x20041000  TX descriptor base (unused by CDC)
;;   0x20041400  TX frame buffer (1536 B, the CDC bulk-OUT staging area)
;;   0x20060000  network state (MAC +0x08, IP +0x18, gateway +0x1C, …)
;;   0x20080000  TCP connection table (ip.lisp conn-base)
;;   0x20090000  USB IRQ ring (unused — no interrupts in this image)
;;   0x20100000  IPC / scratch (ip.lisp locks and counters)
;;
;; Everything is plain identity-mapped DRAM: above the runtime metadata window
;; (0x10000000-0x10200000), below the peripherals (0x3F000000), and raspi3b has
;; a fixed 1 GiB so the addresses are real on QEMU and on hardware.
(defun usb-dma-base ()       #x20000000)
(defun e1000-rx-desc-base () #x20000000)
(defun e1000-rx-buf-base ()  #x20001000)
(defun e1000-tx-desc-base () #x20041000)
(defun e1000-tx-buf-base ()  #x20041400)
(defun e1000-state-base ()   #x20060000)
(defun ssh-conn-base ()      #x20080000)
(defun usb-ring-base ()      #x20090000)
(defun ssh-ipc-base ()       #x20100000)
