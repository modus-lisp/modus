...
raw("(defun dfr0m () (let* ((p (%mmap-exec-page 4096)) (i 0) (w (list 3577218304 (logior 3531603969 (ash (logand p 65535) 5)) (logior 4070572033 (ash (logand (ash p -16) 65535) 5)) 4072669185 4177559584 3548379136 3596551104))) (setf (mem-ref (+ p 256) :u64) 0) (dolist (x w) (setf (mem-ref (+ p i) :u32) x) (setq i (+ i 4))) (%jit-icache-flush p 128) (list (%jit-call p) (mem-ref (+ p 256) :u64))))",3.0)
raw("(defun ceid0m () (let* ((p (%mmap-exec-page 4096)) (i 0) (w (list 3577453760 (logior 3531603969 (ash (logand p 65535) 5)) (logior 4070572033 (ash (logand (ash p -16) 65535) 5)) 4072669185 4177559584 3548379136 3596551104))) (setf (mem-ref (+ p 256) :u64) 0) (dolist (x w) (setf (mem-ref (+ p i) :u32) x) (setq i (+ i 4))) (%jit-icache-flush p 128) (list (%jit-call p) (mem-ref (+ p 256) :u64))))",3.0)
raw("(defun midrm () (let* ((p (%mmap-exec-page 4096)) (i 0) (w (list 3577217024 (logior 3531603969 (ash (logand p 65535) 5)) (logior 4070572033 (ash (logand (ash p -16) 65535) 5)) 4072669185 4177559584 3548379136 3596551104))) (setf (mem-ref (+ p 256) :u64) 0) (dolist (x w) (setf (mem-ref (+ p i) :u32) x) (setq i (+ i 4))) (%jit-icache-flush p 128) (list (%jit-call p) (mem-ref (+ p 256) :u64))))",3.0)
raw("(defun pmcrm () (let* ((p (%mmap-exec-page 4096)) (i 0) (w (list 3577453568 (logior 3531603969 (ash (logand p 65535) 5)) (logior 4070572033 (ash (logand (ash p -16) 65535) 5)) 4072669185 4177559584 3548379136 3596551104))) (setf (mem-ref (+ p 256) :u64) 0) (dolist (x w) (setf (mem-ref (+ p i) :u32) x) (setq i (+ i 4))) (%jit-icache-flush p 128) (list (%jit-call p) (mem-ref (+ p 256) :u64))))",3.0)
ev("(list :dfr0 (dfr0m) :ceid0 (ceid0m) :midr (midrm) :pmcr (pmcrm))",20.0)
print("=== IDM DONE ===",flush=True)
