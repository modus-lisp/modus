;;;; build-ppc32-linux.lisp — hosted Linux/PPC32 image (BIG-ENDIAN).
;;;;
;;;; Usage: sbcl --script mvm/build-ppc32-linux.lisp [source.lisp]
;;;;   Output: /tmp/modus-ppc32-linux (MODUS_PPC32_LINUX_OUT overrides)
;;;;   Run:    qemu-ppc-static /tmp/modus-ppc32-linux
;;;;
...
(install-ppc32-translator)
(ppc-set-linux-mode t)

(let* ((args (rest sb-ext:*posix-argv*))
       (src (first args))
       (out (or (sb-ext:posix-getenv "MODUS_PPC32_LINUX_OUT")
                "/tmp/modus-ppc32-linux"))
       (text (if src
  ...
  (format t "~&Building hosted Linux/PPC32 image...~%")
  (let ((image (build-image :target :linux-ppc32 :source-text text)))
