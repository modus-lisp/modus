...
(defun %install-runtime-cl-macros ()
  ...
  (when (boundp '*modus-runtime-macros*)
    (let ((lst *modus-runtime-macros*))
      ...
      (%rt-install-one (nth 41 lst))
      (%rt-install-one (nth 42 lst))
      (%rt-install-one (nth 43 lst))
      (%rt-install-one (nth 44 lst))
      (%rt-install-one (nth 45 lst))
      (%rt-install-one (nth 46 lst))
      (%rt-install-one (nth 47 lst))
      (%rt-install-one (nth 48 lst))
      (%rt-install-one (nth 49 lst))
      (%rt-install-one (nth 50 lst))
      (%rt-install-one (nth 51 lst))
      (%rt-install-one (nth 52 lst))
      (%rt-install-one (nth 53 lst))
      (%rt-install-one (nth 54 lst))
      (%rt-install-one (nth 55 lst))
      (%rt-install-one (nth 56 lst))
      (%rt-install-one (nth 57 lst))
      (%rt-install-one (nth 58 lst))
      (%rt-install-one (nth 59 lst))
      (%rt-install-one (nth 60 lst))
      (%rt-install-one (nth 61 lst))
      (%rt-install-one (nth 62 lst))
      (%rt-install-one (nth 63 lst))
      (%rt-install-one (nth 64 lst))
      (%rt-install-one (nth 65 lst))
      (%rt-install-one (nth 66 lst))
      (%rt-install-one (nth 67 lst))
      (%rt-install-one (nth 68 lst))
      (%rt-install-one (nth 69 lst))
      (%rt-install-one (nth 70 lst))
      (%rt-install-one (nth 71 lst))
      (%rt-install-one (nth 72 lst))
      (%rt-install-one (nth 73 lst))
      (%rt-install-one (nth 74 lst))
      (%rt-install-one (nth 75 lst))
      (%rt-install-one (nth 76 lst))
      (%rt-install-one (nth 77 lst))
      (%rt-install-one (nth 78 lst))
      (%rt-install-one (nth 79 lst))
      (%rt-install-one (nth 80 lst))
      (%rt-install-one (nth 81 lst))
      (%rt-install-one (nth 82 lst))
      (%rt-install-one (nth 83 lst))
      (%rt-install-one (nth 84 lst))
      (%rt-install-one (nth 85 lst))
      (%rt-install-one (nth 86 lst))
      (%rt-install-one (nth 87 lst))
      (%rt-install-one (nth 88 lst))
      (%rt-install-one (nth 89 lst))
      (%rt-install-one (nth 90 lst))
      (%rt-install-one (nth 91 lst))
      (%rt-install-one (nth 92 lst))
      (%rt-install-one (nth 93 lst))
      (%rt-install-one (nth 94 lst))
      (%rt-install-one (nth 95 lst))
      ;; The unroll above is a FIXED LENGTH.  Entries past it were silently
      ;; never installed: at 46 entries against 41 unrolled calls, PPRINT-
      ;; LOGICAL-BLOCK, PPRINT-POP, DO-ALL-SYMBOLS ... were plain undefined
      ;; functions.  Say so loudly if the list outgrows it again.
      (when (> (length lst) 96)
        (write-string-serial ";; WARNING: *modus-runtime-macros* has more entries than %install-runtime-cl-macros unrolls; the rest are NOT installed")
