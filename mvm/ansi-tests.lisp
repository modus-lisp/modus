...
(defun run-regression-tests ()
  ...
  ;; 8336-8339 — bit-vector-p must reject 2D bit arrays and strings.
  (deftest 8336 (if (bit-vector-p (make-array '(2 2) :element-type 'bit :initial-element 0)) t nil) nil)
  (deftest 8337 (if (bit-vector-p "011100") t nil) nil)
  (deftest 8338 (if (bit-vector-p (make-array 4 :element-type 'bit :initial-element 1)) t nil) t)
  (deftest 8339 (if (bit-vector-p '(0 1 1 0)) t nil) nil)
  (deftest 8340 (if (arrayp (make-array 4 :element-type 'bit :initial-element 1)) t nil) t)
  (deftest 8341 (if (%array-bits-only-p (make-array 4 :element-type 'bit :initial-element 1)) t nil) t)
  (deftest 8342 (if (bit-vector-p #*0010) t nil) t)
  (deftest 8343 (array-rank (make-array 4 :element-type 'bit :initial-element 1)) 1)
  (deftest 8344 (array-rank (make-array '(2 2) :element-type 'bit :initial-element 1)) 2)
...
(defun run-clos-diag-tests ()
  ...
  ;; ==== format-c / ~C char regression probes (9040-9050) ====
  (rt-run-test 9040 (format nil "~C" #\a) "a")
  (rt-run-test 9041 (format nil "~C" (aref "abc" 0)) "a")
  (rt-run-test 9042 (let ((c (aref "abc" 0))) (string= (format nil "~C" c) (string c))) t)
  (rt-run-test 9043 (characterp (aref "abc" 1)) t)
  (rt-run-test 9044 (char-code (aref "abc" 0)) 97)
  (rt-run-test 9045 (let ((s "abc") (out nil))
                      (dotimes (i 3) (push (aref s i) out)) (nreverse out))
               (list #\a #\b #\c))
  (rt-run-test 9046 (string (aref "Xyz" 0)) "X")
  (rt-run-test 9047 (format nil "~:C" #\Space) "Space")
  (rt-run-test 9048 (format nil "~@C" #\a) "#\\a")
  (rt-run-test 9049 (eql (aref "abc" 0) #\a) t)
  ;; ==== print-symbols matrix probes (9050-9058) ====
  ;; gensym #: prefix (print.symbol.prefix.1)
  (rt-run-test 9050 (write-to-string (make-symbol "ABC")
                                     :gensym t :case :upcase :escape t :readably nil)
               "#:ABC")
  ;; gensym suppressed (print.symbol.prefix.2)
  (rt-run-test 9051 (write-to-string (make-symbol "ABC")
                                     :gensym nil :case :upcase :readably nil :escape nil)
               "ABC")
  ;; gensym suppressed but escape t (print.symbol.prefix.4)
  (rt-run-test 9052 (write-to-string (make-symbol "ABC")
                                     :gensym nil :case :upcase :readably nil :escape t)
               "ABC")
  ;; lowercase-named symbol under default (:upcase) readtable-case + escape
  ;; needs |x| or \x (prin1.symbol.2 shape).
  (rt-run-test 9053 (let ((*print-escape* t) (*print-readably* nil))
                      (let ((s (prin1-to-string '|x|)))
                        (if (or (string= s "|x|") (string= s "\\x")) t s)))
               t)
  ;; downcase symbol with escape OFF: princ prints raw name
  (rt-run-test 9054 (let ((*print-escape* nil)) (princ-to-string '|x|)) "x")
  ;; external symbol prints with single colon when qualified
  (rt-run-test 9055 (let ((*print-escape* t))
