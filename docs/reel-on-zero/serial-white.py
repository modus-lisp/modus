...
ev("(let* ((b (first *rh-bufs*)) (ysz (rh-ceil64 (* 384 244))) (usz (rh-ceil64 (* 192 122)))) (hvs-nfill-nc b ysz #xEBEBEBEB) (hvs-nfill-nc (+ b ysz) usz #x80808080) (hvs-nfill-nc (+ b ysz usz) usz #x80808080) (list ysz usz (mem-ref (+ b 12320) :u8) (mem-ref (+ b ysz 3088) :u8)))",60.0)
subprocess.run(["ffmpeg","-hide_banner","-loglevel","error","-f","v4l2","-input_format","mjpeg","-video_size","1280x720","-i","/dev/video0","-frames:v","3","-y","/home/modus/white-%d.jpg"])
ev("(let ((y (reel.decode::picture-y *pic*)) (u (reel.decode::picture-u *pic*))) (list (length y) (length u)))",30.0)
